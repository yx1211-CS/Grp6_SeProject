...
export default function ModeratorDashboard() {
  ...
  const fetchReports = async () => {
    ...
    if (error) {
      ...
      // --- Calculate Stats ---
      const highRiskCount = groupedData.filter(
        (item) => item.reportCount >= 3,
      ).length;
      const hiddenCount = groupedData.filter(
        (item) => item.post?.ishidden,
      ).length;
  ...
  const getDisplayData = () => {
    ...
    if (activeFilter === "HighRisk") {
      ...
      data = data.filter(
        (item) => item.post?.ishidden || item.reportCount >= 3,
      );
    }
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          ...
          isActive && styles.activeStatsCard,
        ]}
      >
        ...
        <Text
          style={[
            styles.statsLabel,
            isActive && { fontWeight: "bold", color: color },
          ]}
        >
          {label}
        </Text>
  ...
  const renderItem = ({ item }) => {
    ...
    const isHighRisk = item.reportCount >= 3;
          ...
          {isHighRisk && (
            <View style={styles.riskBadge}>
              <Text style={styles.riskText}>
                ⚠️ High Risk ({item.reportCount} reports)
              </Text>
            </View>
          )}
          {!isHighRisk && (
            <Text style={styles.reasonLabel}>
              Reasons: {item.combinedReasons}
            </Text>
          )}
        </View>
      </TouchableOpacity>
              ...
              <View style={styles.sortBtnRow}>
                {["Newest", "Oldest", "Most Reported"].map((opt) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.sortBtn,
                      sortOption === opt && styles.activeSortBtn,
                    ]}
                  >
                    <Text
                      style={[
                        styles.sortBtnText,
                        sortOption === opt && styles.activeSortBtnText,
                      ]}
                    >
                      {opt}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  activeStatsCard: { borderWidth: 2, transform: [{ scale: 1.02 }] },
